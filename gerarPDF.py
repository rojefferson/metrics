...
def carregar_template(nome_template: str):
    ...
    env = Environment(
        loader=FileSystemLoader('c:\\Users\\escon\\Downloads\\krol\\metrics\\'),
        autoescape=False
    )
...
def montar_dados(
    df_list: List[Tuple[str, pd.DataFrame]],
    client_name: str,
    campaign_name: str
) -> dict:
    """
    Monta o dicionário de dados para o template Jinja2 a partir
    da lista de dataframes por plataforma.
    df_list: [(nome_plataforma, df), ...]
    """
    plataformas = []
    for platform_name, df in df_list:
...
def gerar_relatorio(
    ...
    """Pipeline completo: recebe lista de (plataforma, DataFrame) -> monta o dicionário de dados -> gera .md -> converte para .pdf"""
    dados = montar_dados(df_list, client_name, campaign_name)

    # slug pra nome de arquivo
    slug = client_name.replace(" ", "_")
    if campaign_name:
        slug += "_" + campaign_name.replace(" ", "_")
